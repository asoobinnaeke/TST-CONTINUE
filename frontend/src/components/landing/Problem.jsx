...
export default function Problem() {
  return (
    <section
      ...
      className="relative py-24 lg:py-32 bg-[#0B0B0F] border-t border-white/10"
    >
      <div className="max-w-7xl mx-auto px-5 lg:px-8">
        <div className="max-w-2xl mb-14">
          ...
          <h2 className="mt-3 text-4xl md:text-5xl font-bold tracking-tight text-white leading-[1.05]">
            ...
            <span className="text-white/45">just two options.</span>
          </h2>
          <p className="mt-4 text-[15px] text-white/65 max-w-xl leading-relaxed">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {cards.map((c, i) => (
            <div
              ...
              className={`group relative rounded-3xl border p-7 hover:-translate-y-1 hover:shadow-[0_18px_36px_-12px_rgba(0,0,0,0.5)] transition-all ${c.tone === "purple" ? "bg-[#0F0F12] border-[#0F0F12] text-white" : "bg-[#16161D] border-white/10"}`}
            >
              {c.badge && (
                <div className="absolute -top-3 left-7 text-[10px] font-bold uppercase tracking-wider bg-[#B4E04C] text-white px-2.5 py-1 rounded-full inline-flex items-center gap-1">
              ...
              <div
                className={`w-11 h-11 rounded-xl grid place-items-center mb-5 ${
                  c.tone === "loss"
                    ...
                    : c.tone === "purple"
                    ? "bg-[#B4E04C] text-white"
                    : "bg-[#16161D]/5 text-white/85"
                }`}
              >
                <c.icon className="w-5 h-5" strokeWidth={2} />
              </div>
              <div className={`text-xs font-mono uppercase tracking-[0.18em] ${c.tone === "purple" ? "text-white/50" : "text-white/45"}`}>
                {c.tag}
              </div>
              <h3 className={`mt-2 text-xl font-semibold ${c.tone === "purple" ? "text-white" : "text-white"}`}>{c.title}</h3>
              <p className={`mt-3 text-[15px] leading-relaxed ${c.tone === "purple" ? "text-white/70" : "text-white/65"}`}>{c.body}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-14 flex items-center justify-center">
          <div className="inline-flex items-center gap-3 bg-[#0F0F12] text-white rounded-full pl-2 pr-5 py-2">
            <span className="inline-flex items-center gap-1.5 bg-[#B4E04C] text-white rounded-full px-3 py-1 text-xs font-semibold">
